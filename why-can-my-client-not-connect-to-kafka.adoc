...
2. The client then connects to one of the brokers *returned in the first step* according to which broker is the leader for the partition that the client is producing or consuming to or from.
...
Below I show using a client connecting to Kafka in various permutations of deployment topology. I'm using Python with librdkafka (`confluent_kafka`) but the principle applies to clients across all languages. The python code I'm running is very simple and just serves to illustrate the connection process. It's simplified to benefit clarity, at the expense of good coding and functionality :) 

[source,python]
----
include::python_kafka_test_client.py[]
----
...
.docker-compose.yml
----
include::docker-compose_initial.yml[]
----
...
Nope - any client application should be able to expose the metadata too. <<TODO link to client library list here>> . Here's an example using kafkacat: 
